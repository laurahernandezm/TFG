...
def clustering (cells_summary, file_preffix):
    ...
    #Scale matrix values
    cells_summary_matrix_scaled = scale(cells_summary_matrix, 1)
    ...
    clusters = AffinityPropagation(damping=0.9, preference=-300, max_iter=1000).fit(cells_summary_matrix_scaled)
                ...
                for possible_zone in added_zones[clusters.labels_[i]]:
                    ...
                    if (not added and possible_zone != last_zone_index):
                        for cell_j in zones[possible_zone]:
                            ...
                            if (cell_j == cells_ids[i]-NUM_CELLS_VER or cell_j == cells_ids[i]+NUM_CELLS_VER or
                                cell_j == cells_ids[i]-1 or cell_j == cells_ids[i]+1 or
                                cell_j == cells_ids[i]+NUM_CELLS_VER-1 or cell_j == cells_ids[i]-NUM_CELLS_VER+1 or
                                cell_j == cells_ids[i]+NUM_CELLS_VER+1 or cell_j == cells_ids[i]-NUM_CELLS_VER-1):
